...
public final class PasswordUtil {
    ...
    private static boolean verifyModernHash(String inputPassword, String storedHash) {
        HashParts hashParts = parseModernHash(storedHash);
        if (hashParts == null) return false;
        byte[] derivedKey = deriveKey(inputPassword.toCharArray(), hashParts.salt, hashParts.iterations, hashParts.hash.length);
    ...
    private static HashParts parseModernHash(String storedHash) {
        ...
        if (parts.length != 4 || !HASH_PREFIX.equals(parts[0])) return null;
        try {
            int iterations = Integer.parseInt(parts[1]);
            byte[] salt = Base64.getDecoder().decode(parts[2]);
            byte[] hash = Base64.getDecoder().decode(parts[3]);
            if (iterations < 1 || salt.length == 0 || hash.length == 0) return null;
