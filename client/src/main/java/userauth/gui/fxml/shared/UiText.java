...
public final class UiText {
    ...
    public static String auctionStatus(AuctionStatus status) {
        if (status == null) {
            return "";
        }
    ...
    public static void configureTranslatedComboBox(ComboBox<String> comboBox) {
        if (comboBox == null) {
            return;
        }
    ...
    public static void refreshTranslatedComboBox(ComboBox<String> comboBox) {
        if (comboBox == null) {
            return;
        }
    ...
    private static Map<String, String> createPrefixTranslations() {
        ...
        put(values, "Won products: ", "Sản phẩm thắng: ");
        put(values, "Bidder: ", "Người đấu giá: ");
        put(values, "User ID: ", "ID người dùng: ");
        put(values, "Price: ", "Giá: ");
        put(values, "At: ", "Lúc: ");
        put(values, "SUCCESS: Transaction ID ", "THÀNH CÔNG: Mã giao dịch ");
        ...
        put(values, "An unexpected error occurred while updating the auto-bid rule: ", "Đã xảy ra lỗi khi cập nhật quy tắc tự động đấu giá: ");
        put(values, "An unexpected error occurred while deleting the auto-bid rule: ", "Đã xảy ra lỗi khi xóa quy tắc tự động đấu giá: ");
        put(values, "Cannot connect to auction server ", "Không thể kết nối tới máy chủ đấu giá ");
        put(values, "Insufficient available wallet balance. Available funds for this bid: ", "Số dư khả dụng trong ví không đủ. Số tiền có thể dùng cho lượt đấu giá này: ");
        put(values, "Insufficient wallet balance to reserve ", "Số dư ví không đủ để giữ ");
        put(values, "Reserved wallet balance is lower than the amount to capture for user ID: ", "Số tiền đang giữ nhỏ hơn số tiền cần thu của người dùng ID: ");
        put(values, "Reserved wallet balance is lower than the amount to release for user ID: ", "Số tiền đang giữ nhỏ hơn số tiền cần giải phóng của người dùng ID: ");
        put(values, "Top-up amount must be at least ", "Số tiền nạp phải ít nhất ");
        put(values, "Top-up amount must not exceed ", "Số tiền nạp không được vượt quá ");
        put(values, "Unable to create or retrieve wallet for user ID: ", "Không thể tạo hoặc lấy ví của người dùng ID: ");
        put(values, "Unable to delete the auto bid by auction/bidder in database: ", "Không thể xóa tự động đấu giá theo phiên/người đấu giá trong cơ sở dữ liệu: ");
        put(values, "Unable to delete the auto bid in database: ", "Không thể xóa tự động đấu giá trong cơ sở dữ liệu: ");
        put(values, "Unable to find the auto bid in database: ", "Không thể tìm tự động đấu giá trong cơ sở dữ liệu: ");
        put(values, "Unable to initialize the database connection to ", "Không thể khởi tạo kết nối cơ sở dữ liệu tới ");
        put(values, "Unable to read all auto bids from database: ", "Không thể đọc toàn bộ tự động đấu giá từ cơ sở dữ liệu: ");
        put(values, "Unable to read auto bids by auction from database: ", "Không thể đọc tự động đấu giá theo phiên từ cơ sở dữ liệu: ");
        put(values, "Unable to release wallet funds while cancelling auction ", "Không thể giải phóng tiền trong ví khi hủy phiên ");
        put(values, "Unable to save the auto bid to database: ", "Không thể lưu tự động đấu giá vào cơ sở dữ liệu: ");
        put(values, "Unable to update the auction in PostgreSQL. SQLState=", "Không thể cập nhật phiên trong PostgreSQL. SQLState=");
        put(values, "Unable to update the auto bid in database: ", "Không thể cập nhật tự động đấu giá trong cơ sở dữ liệu: ");
        put(values, "Unsupported network action: ", "Hành động mạng không được hỗ trợ: ");
        put(values, "Unsupported settlement status: ", "Trạng thái xử lý kết quả không được hỗ trợ: ");
        put(values, "Wallet already exists for user ID: ", "Ví đã tồn tại cho người dùng ID: ");
    ...
    private static void addCommonTranslations(Map<String, String> values) {
        ...
        put(values, "Account", "Tài khoản");
        put(values, "ACCOUNT INFORMATION", "THÔNG TIN TÀI KHOẢN");
        put(values, "SECURITY SETTINGS", "CÀI ĐẶT BẢO MẬT");
        put(values, "ID", "MÃ");
        ...
        put(values, "Category", "Danh mục");
        put(values, "Time", "Thời gian");
        put(values, "Result", "Kết quả");
        put(values, "Bid Amount", "Giá đã đặt");
        put(values, "Winning Price", "Giá thắng");
        put(values, "Ended At", "Kết thúc lúc");
        put(values, "Transaction History", "Lịch sử giao dịch");
        put(values, "Won Products", "Sản phẩm đã thắng");
        put(values, "Won products", "Sản phẩm đã thắng");
        put(values, "No won products yet.", "Chưa có sản phẩm thắng đấu giá.");
        put(values, "Unknown auction", "Phiên không xác định");
        put(values, "Leading", "Đang dẫn đầu");
        put(values, "Won", "Đã thắng");
        put(values, "Outbid", "Đã bị vượt giá");
        put(values, "Pending", "Đang chờ");
        ...
        put(values, "Updated at: -", "Cập nhật lúc: -");
        put(values, "Bidder: -", "Người đấu giá: -");
        put(values, "Total transactions: 0 | Won products: 0", "Tổng giao dịch: 0 | Sản phẩm thắng: 0");
        ...
        put(values, "SELLER", "NGƯỜI BÁN");
        put(values, "BIDDER", "NGƯỜI ĐẤU GIÁ");
        put(values, "Role: ADMIN", "Vai trò: Quản trị");
        put(values, "Role: SELLER", "Vai trò: Người bán");
        put(values, "Role: BIDDER", "Vai trò: Người đấu giá");
        ...
        put(values, "0/3 | closes at -", "0/3 | đóng lúc -");
        ...
        put(values, "WALLET", "VÍ");
        put(values, "TOTAL", "TỔNG");
        put(values, "AVAILABLE", "KHẢ DỤNG");
        put(values, "RESERVED", "ĐANG GIỮ");
        put(values, "AMOUNT", "SỐ TIỀN");
        put(values, "PAYMENT METHOD", "PHƯƠNG THỨC THANH TOÁN");
        put(values, "Credit Card", "Thẻ tín dụng");
        put(values, "Bank Transfer", "Chuyển khoản ngân hàng");
        put(values, "E-Wallet", "Ví điện tử");
        put(values, "Cash", "Tiền mặt");
    ...
    private static void addDashboardTranslations(Map<String, String> values) {
        ...
        put(values, "Create, preview, and publish homepage announcements from one workspace.", "Tạo, xem trước và đăng thông báo trang chủ trong một khu làm việc.");
        put(values, "Manage homepage announcements and linked auctions from one admin screen.", "Quản lý thông báo trang chủ và phiên liên kết trên một màn hình quản trị.");
        put(values, "Draft content, bind it to auctions, and review the live preview before publishing.", "Soạn nội dung, liên kết với phiên đấu giá và xem trước trước khi đăng.");
        put(values, "Use this form to publish new homepage content or update an existing announcement.", "Dùng biểu mẫu này để đăng nội dung trang chủ mới hoặc cập nhật thông báo hiện có.");
        put(values, "Select an item to edit or remove published homepage content.", "Chọn một mục để sửa hoặc gỡ nội dung trang chủ đã đăng.");
        put(values, "Choose an auction here, then pull its schedule into the announcement form.", "Chọn một phiên tại đây rồi đưa lịch của phiên vào biểu mẫu thông báo.");
        ...
        put(values, "HOMEPAGE CMS", "CMS TRANG CHỦ");
        put(values, "HOMEPAGE OPERATIONS", "THAO TÁC TRANG CHỦ");
        ...
        put(values, "MARK AS PAID", "ĐÁNH DẤU ĐÃ THANH TOÁN");
        put(values, "CANCEL RESULT", "HỦY KẾT QUẢ");
        put(values, "PLACE BID NOW", "ĐẶT GIÁ NGAY");
        put(values, "SAVE AUTO-BID", "LƯU TỰ ĐẤU GIÁ");
        put(values, "REFRESH USERS", "LÀM MỚI NGƯỜI DÙNG");
        put(values, "REFRESH AUCTIONS", "LÀM MỚI PHIÊN");
        put(values, "LOCK / UNLOCK", "KHÓA / MỞ KHÓA");
        put(values, "DELETE ACCOUNT", "XÓA TÀI KHOẢN");
        ...
        put(values, "STARTING PRICE", "GIÁ KHỞI ĐIỂM");
        put(values, "Start Date & Time", "Ngày và giờ bắt đầu");
        put(values, "End Date & Time", "Ngày và giờ kết thúc");
        put(values, "START TIME", "GIỜ BẮT ĐẦU");
        put(values, "END TIME", "GIỜ KẾT THÚC");
        put(values, "ANTI-SNIPING", "CHỐNG SNIPE");
        put(values, "Auto Bidding (Current auction)", "Tự động đấu giá (phiên hiện tại)");
        put(values, "Autobids List", "Danh sách tự động đấu giá");
        put(values, "New auto-bid rule", "Quy tắc tự động đấu giá mới");
        put(values, "Max price", "Giá tối đa");
        put(values, "Increment", "Bước tăng");
        put(values, "Enter product name", "Nhập tên sản phẩm");
        put(values, "Enter max price", "Nhập giá tối đa");
        put(values, "Enter increment", "Nhập bước tăng");
        ...
        put(values, "TOP UP", "NẠP TIỀN");
        put(values, "TOP UP WALLET", "NẠP TIỀN VÀO VÍ");
        put(values, "Enter amount to top up", "Nhập số tiền cần nạp");
    }
    ...
    private static void addFormTranslations(Map<String, String> values) {
        ...
        put(values, "Confirm New Password", "Xác nhận mật khẩu mới");
        put(values, "Enter current password", "Nhập mật khẩu hiện tại");
        put(values, "Enter new password", "Nhập mật khẩu mới");
        put(values, "Re-enter your new password", "Nhập lại mật khẩu mới");
        put(values, "Update Password", "Cập nhật mật khẩu");
        put(values, "Password changes take effect immediately.", "Thay đổi mật khẩu có hiệu lực ngay lập tức.");
        put(values, "Update your password to keep your account secure. Use a strong password with both letters and numbers.", "Cập nhật mật khẩu để bảo vệ tài khoản. Hãy dùng mật khẩu mạnh có cả chữ và số.");
        put(values, "At least 6 characters, include letters and numbers", "Tối thiểu 6 ký tự, gồm chữ và số");
        put(values, "Changes are saved to your account immediately.", "Thay đổi được lưu ngay vào tài khoản của bạn.");
        put(values, "Keep your contact details up to date to receive auction notifications and account updates.", "Luôn cập nhật thông tin liên hệ để nhận thông báo đấu giá và cập nhật tài khoản.");
    ...
    private static void addRuntimeTranslations(Map<String, String> values) {
        ...
        put(values, "Are you sure you want to permanently delete this account?", "Bạn có chắc muốn xóa vĩnh viễn tài khoản này không?");
        ...
        put(values, "Please enter a bid amount and increment.", "Vui lòng nhập mức giá đấu và bước tăng.");
        ...
        put(values, "Invalid number.", "Số không hợp lệ.");
        put(values, "Invalid auto-bid id.", "Mã tự động đấu giá không hợp lệ.");
        put(values, "AutoBid is not ready.", "Chức năng tự động đấu giá hiện chưa sẵn sàng.");
        put(values, "Please select an auto-bid rule.", "Vui lòng chọn một quy tắc tự động đấu giá.");
        put(values, "Wallet is not ready.", "Ví hiện chưa sẵn sàng.");
        put(values, "Connect this controller to AuthFrame to open wallet top-up.", "Hãy kết nối controller này với AuthFrame để mở nạp tiền vào ví.");
        put(values, "This auction does not have an image to preview.", "Phiên này không có ảnh để xem trước.");
        put(values, "Click anywhere to close", "Bấm vào bất kỳ đâu để đóng");
        put(values, "Editing auto-bid", "Đang sửa tự động đấu giá");
        ...
        put(values, "Auction settlement is not ready.", "Chức năng xử lý kết quả phiên hiện chưa sẵn sàng.");
        put(values, "Invalid amount format.", "Định dạng số tiền không hợp lệ.");
        put(values, "Not enough information to process top-up.", "Không đủ thông tin để xử lý nạp tiền.");
        put(values, "Please enter the top-up amount.", "Vui lòng nhập số tiền cần nạp.");
        put(values, "Please select a payment method.", "Vui lòng chọn phương thức thanh toán.");
        put(values, "Top-up successful. Transaction ID: ", "Nạp tiền thành công. Mã giao dịch: ");
        put(values, "Unable to process the top-up request right now.", "Hiện không thể xử lý yêu cầu nạp tiền.");
    ...
    private static void addServiceTranslations(Map<String, String> values) {
        ...
        put(values, "Account for deletion was not found.", "Không tìm thấy tài khoản để xóa.");
        put(values, "Admin accounts cannot be created from the registration screen.", "Không thể tạo tài khoản quản trị từ màn hình đăng ký.");
        ...
        put(values, "Auction marked as paid.", "Phiên đã được đánh dấu là đã thanh toán.");
        put(values, "Auction not found.", "Không tìm thấy phiên đấu giá.");
        put(values, "Auction result has been cancelled.", "Kết quả phiên đã được hủy.");
        put(values, "Auction schedule information cannot be empty.", "Thông tin lịch phiên không được để trống.");
        put(values, "A finished auction without a winner cannot be marked as paid.", "Phiên đã kết thúc nhưng không có người thắng nên không thể đánh dấu đã thanh toán.");
        put(values, "AutoBid item not found.", "Không tìm thấy mục tự động đấu giá.");
        ...
        put(values, "Database connection is closed.", "Kết nối cơ sở dữ liệu đã đóng.");
        ...
        put(values, "Image file is too large. Maximum supported size is 5 MB.", "Tệp ảnh quá lớn. Kích thước tối đa được hỗ trợ là 5 MB.");
        put(values, "Increment cannot be greater than max price.", "Bước tăng không được lớn hơn giá tối đa.");
        put(values, "Increment must be greater than 0", "Bước tăng phải lớn hơn 0");
        ...
        put(values, "Interrupted while waiting for a database connection.", "Đã bị ngắt khi đang chờ kết nối cơ sở dữ liệu.");
        put(values, "Item not found.", "Không tìm thấy mục.");
        put(values, "Max price must be greater than 0.", "Giá tối đa phải lớn hơn 0.");
        ...
        put(values, "Only admins can delete accounts.", "Chỉ quản trị viên mới được xóa tài khoản.");
        ...
        put(values, "Only the creator can settle this auction.", "Chỉ người tạo mới được xử lý kết quả phiên này.");
        put(values, "Only finished auctions can move to PAID.", "Chỉ phiên đã kết thúc mới có thể chuyển sang đã thanh toán.");
        put(values, "Only finished or paid auctions can be cancelled.", "Chỉ phiên đã kết thúc hoặc đã thanh toán mới có thể bị hủy.");
        put(values, "Password hashing failed", "Băm mật khẩu thất bại");
        put(values, "Payment method is required.", "Vui lòng chọn phương thức thanh toán.");
        ...
        put(values, "Reserved amount must be greater than 0.", "Số tiền đang giữ phải lớn hơn 0.");
        put(values, "Bid amount must be greater than 0.", "Mức giá đấu phải lớn hơn 0.");
        put(values, "Refund amount must be greater than 0.", "Số tiền hoàn phải lớn hơn 0.");
        put(values, "Expected reserved balance cannot be negative.", "Số dư đang giữ kỳ vọng không được âm.");
        put(values, "Reserved balance cannot be negative.", "Số dư đang giữ không được âm.");
        put(values, "Reserved balance cannot exceed the wallet balance.", "Số dư đang giữ không được vượt quá số dư ví.");
        ...
        put(values, "Top-up amount must be a multiple of 1000 VND.", "Số tiền nạp phải là bội số của 1000 VND.");
        put(values, "Unable to create top-up request.", "Không thể tạo yêu cầu nạp tiền.");
        put(values, "Unable to decompress image data.", "Không thể giải nén dữ liệu ảnh.");
        put(values, "Unable to delete the user from PostgreSQL.", "Không thể xóa người dùng khỏi PostgreSQL.");
        put(values, "Unable to find top-up transaction.", "Không tìm thấy giao dịch nạp tiền.");
        put(values, "Unable to find wallet.", "Không tìm thấy ví.");
        put(values, "Unable to read generated top-up id.", "Không thể đọc mã nạp tiền vừa tạo.");
        put(values, "Unable to read generated wallet id.", "Không thể đọc mã ví vừa tạo.");
        put(values, "Unable to read top-up history.", "Không thể đọc lịch sử nạp tiền.");
        put(values, "Unable to read wallet.", "Không thể đọc ví.");
        put(values, "Unable to read wallet transactions.", "Không thể đọc giao dịch ví.");
        put(values, "Unable to save top-up transaction.", "Không thể lưu giao dịch nạp tiền.");
        put(values, "Unable to save wallet.", "Không thể lưu ví.");
        put(values, "Unable to save wallet transaction.", "Không thể lưu giao dịch ví.");
        put(values, "Unable to update top-up transaction.", "Không thể cập nhật giao dịch nạp tiền.");
        put(values, "Unable to update wallet.", "Không thể cập nhật ví.");
        put(values, "Unexpected top-up history validation failure.", "Xác thực lịch sử nạp tiền gặp lỗi ngoài dự kiến.");
        put(values, "Unexpected wallet lookup failure.", "Tra cứu ví gặp lỗi ngoài dự kiến.");
        ...
        put(values, "You cannot delete your own account.", "Bạn không thể xóa chính tài khoản của mình.");
        put(values, "You are already the leading bidder for this auction.", "Bạn đang là người dẫn đầu phiên này.");
        ...
        put(values, "Wallet balance cannot be negative.", "Số dư ví không được âm.");
    }
