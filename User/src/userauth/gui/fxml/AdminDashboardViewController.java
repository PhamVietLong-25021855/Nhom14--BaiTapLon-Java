...
public class AdminDashboardViewController {
    ...
    private void handleDeleteUser() {
        ...
        User selected = getSelectedUser("Please select a bidder or seller account to delete.");
        if (selected == null) {
            return;
        }
        if (selected.getRole() == Role.ADMIN) {
            NotificationUtil.warning(ownerWindow(), "Notification", "Admin accounts cannot be deleted.");
            return;
        }

        boolean confirmed = NotificationUtil.confirm(ownerWindow(), "Confirm", "Are you sure you want to permanently delete this account?");
        if (!confirmed) {
            return;
        }
        ...
        int selectedUserId = selected.getId();
        runActionAsync(
                () -> authController.deleteUser(currentUser.getUsername(), selectedUserId),
    ...
    private void applyAdminSnapshot(AdminSnapshot snapshot, int selectedUserId, int selectedAuctionId) {
        ...
        updateUserSelectionSummary(selectedUser);
        updateUserActionState(selectedUser);
    ...
    private void registerSelectionListeners() {
        tableUsers.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            ...
            updateUserActionState(newValue);
        });
    ...
    private void updateUserActionState(User selected) {
        if (btnDeleteUser == null) {
            return;
        }
        btnDeleteUser.setDisable(selected == null || selected.getRole() == Role.ADMIN);
    }
    ...
    private void setActionBusy(boolean busy) {
        ...
        if (!busy) {
            updateUserActionState(tableUsers == null ? null : tableUsers.getSelectionModel().getSelectedItem());
        }
    }
