...
public class ChangePasswordDialogController {
    ...
    private void handleSubmit() {
        ...
        if (!UserValidator.isValidPassword(newPassword)) {
            showError(UiText.text("New password must be at least 6 characters and include letters and numbers."));
            return;
        }
        if (!newPassword.equals(confirmPassword)) {
            showError(UiText.text("Password confirmation does not match."));
            return;
        }
    ...
    private void setBusy(boolean busy) {
        ...
        if (txtConfirmPassword != null) {
            txtConfirmPassword.setDisable(busy);
        }
    }
}
