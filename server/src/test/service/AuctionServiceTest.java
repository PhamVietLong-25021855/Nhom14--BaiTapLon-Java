...
class AuctionServiceTest {
    ...
    @Test
    void placeBidRejectsCurrentLeaderFromBiddingAgainstSelf() {
        AuctionItem item = runningAuction(1, 100.0, 150.0);
        item.setWinnerId(20);
        auctionDao.saveAuction(item);

        InvalidBidException ex = assertThrows(InvalidBidException.class,
                () -> auctionService.placeBid(1, 20, 160.0));

        assertEquals("You are already the leading bidder for this auction.", ex.getMessage());
        assertEquals(150.0, auctionDao.findAuctionById(1).getCurrentHighestBid());
