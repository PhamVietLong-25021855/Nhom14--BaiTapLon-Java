...
final class AuctionSettlementHandlerFactory {
    ...
    private static final class PaidSettlementHandler implements AuctionSettlementHandler {
        @Override
        public AuctionStatus targetStatus() {
            return AuctionStatus.PAID;
        }

        @Override
        public void validate(AuctionItem item) throws ValidationException {
        ...
        @Override
        public void apply(AuctionItem item, long now) {
            item.setStatus(AuctionStatus.PAID);
            item.setUpdatedAt(now);
        }

        @Override
        public String summary(AuctionItem item) {
            return "Auction marked as paid.";
        }
    }
    ...
    private static final class CanceledSettlementHandler implements AuctionSettlementHandler {
        ...
        @Override
        public void validate(AuctionItem item) {
        }
        ...
        @Override
        public String summary(AuctionItem item) {
            return "Auction result has been cancelled.";
        }
    }
}
